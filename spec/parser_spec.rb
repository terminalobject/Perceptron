...
describe Parser do
  ...
  describe '#parse' do
    context 'Inputs considered bad' do
      ...
      let(:first_output)  { { vector: [1, 1, 2, 0], expected: 1 } }
      let(:second_output) { { vector: [1, 1, 3, 0], expected: 1 } }
    ...
    context 'Inputs considered good' do
      ...
      let(:first_output)  { { vector: [1, 0, 0, 0], expected: 0 } }
      let(:second_output) { { vector: [1, 0, 0, 0], expected: 0 } }
      let(:output)        { [first_output, second_output] }
